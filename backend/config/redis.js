  ...
  socket: {
    reconnectStrategy: (retries) => {
      ...
      const delay = Math.min(
        RETRY_DELAY_BASE * Math.pow(2, retries) + Math.random() * 2000, 
        60000 // 最大延迟60秒
      )
  ...
  try {
    ...
    redisClient.on('error', (err) => {
      ...
      if (err.code === 'ECONNREFUSED') {
        logger.error('[REDIS] Redis服务器拒绝连接，请检查Redis服务是否正在运行')
      } else if (err.code === 'ETIMEDOUT') {
        logger.error('[REDIS] 连接超时，请检查网络连接和Redis服务器状态')
      } else {
        logger.error('[REDIS] 发生未知错误')
      }
    })
