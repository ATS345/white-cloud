...
export const validateCreateGame = [
  ...
  body('discount_price')
    ...
    .isFloat({ min: 0 })
    .withMessage('折扣价不能小于0'),

  body('release_date')
    .notEmpty().withMessage('发布日期不能为空')
    .isISO8601()
    .withMessage('发布日期必须是有效的ISO 8601格式'),

  body('main_image_url')
    .notEmpty().withMessage('主图URL不能为空')
    .isURL()
    .withMessage('主图URL必须是有效的URL地址'),
...
export const validateCreateGameVersion = [
  ...
  body('version_number')
    ...
    .isString()
    .withMessage('版本号必须是字符串'),

  body('file_url')
    .notEmpty().withMessage('文件URL不能为空')
    .isURL()
    .withMessage('文件URL必须是有效的URL地址'),

  body('file_size')
    .notEmpty().withMessage('文件大小不能为空')
    .isNumeric()
    .withMessage('文件大小必须是数字')
    .isFloat({ min: 0 })
    .withMessage('文件大小不能小于0'),

  body('platform')
    .notEmpty().withMessage('平台不能为空')
    .isString()
    .withMessage('平台必须是字符串')
    .isIn(['windows', 'mac', 'linux'])
    .withMessage('平台必须是windows、mac或linux'),
...
export const validateCreateSystemRequirement = [
  ...
  body('os')
    ...
    .isString()
    .withMessage('操作系统必须是字符串'),

  body('processor')
    .notEmpty().withMessage('处理器不能为空')
    .isString()
    .withMessage('处理器必须是字符串'),

  body('memory')
    .notEmpty().withMessage('内存不能为空')
    .isString()
    .withMessage('内存必须是字符串'),

  body('graphics')
    .notEmpty().withMessage('显卡不能为空')
    .isString()
    .withMessage('显卡必须是字符串'),

  body('storage')
    .notEmpty().withMessage('存储不能为空')
    .isString()
    .withMessage('存储必须是字符串'),

  body('type')
    .optional()
    .isString().withMessage('需求类型必须是字符串')
    .isIn(['minimum', 'recommended'])
    .withMessage('需求类型必须是minimum或recommended'),
];
...
export const validateSyncGameData = [
  ...
  body('data_type')
    ...
    .isIn(['save_data', 'settings', 'progress'])
    .withMessage('数据类型必须是save_data、settings或progress'),
...
export const validateGetGames = [
  ...
  query('minPrice')
    ...
    .isFloat({ min: 0 })
    .withMessage('最低价格不能小于0'),

  query('maxPrice')
    .optional()
    .isNumeric().withMessage('最高价格必须是数字')
    .isFloat({ min: 0 })
    .withMessage('最高价格不能小于0'),

  query('sortBy')
    .optional()
    .isString().withMessage('排序字段必须是字符串')
    .isIn(['release_date', 'price', 'title', 'rating'])
    .withMessage('排序字段必须是release_date、price、title或rating'),
