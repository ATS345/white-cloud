...
export const clearCacheByPattern = async (pattern) => {
  try {
    ...
    if (redisClient.keys) {
      ...
      for (const key of redisClient._cache.keys()) {
        if (regex.test(key)) {
...
// 生成游戏列表缓存键
export const generateGameListCacheKey = (options) => {
  return generateCacheKey(CACHE_PREFIXES.GAME_LIST, 'all', {
    page: options.page || 1,
...
// 生成游戏详情缓存键
export const generateGameDetailCacheKey = (gameId) => {
  return generateCacheKey(CACHE_PREFIXES.GAME_DETAIL, gameId)
}

// 生成游戏分类缓存键
export const generateGameCategoriesCacheKey = () => {
  return generateCacheKey(CACHE_PREFIXES.GAME_CATEGORIES, 'all')
}

// 生成游戏标签缓存键
export const generateGameTagsCacheKey = () => {
  return generateCacheKey(CACHE_PREFIXES.GAME_TAGS, 'all')
}

// 生成游戏评论缓存键
export const generateGameReviewsCacheKey = (gameId, options) => {
  return generateCacheKey(CACHE_PREFIXES.GAME_REVIEWS, gameId, {
    page: options.page || 1,
...
// 生成按分类查询游戏缓存键
export const generateGamesByCategoryCacheKey = (categoryName, options) => {
  return generateCacheKey(CACHE_PREFIXES.GAME_BY_CATEGORY, categoryName, {
    page: options.page || 1,
    limit: options.limit || 20
  })
}
...
// 生成按标签查询游戏缓存键
export const generateGamesByTagCacheKey = (tagName, options) => {
  return generateCacheKey(CACHE_PREFIXES.GAME_BY_TAG, tagName, {
    page: options.page || 1,
    limit: options.limit || 20
  })
}
...
// 生成系统需求缓存键
export const generateSystemRequirementsCacheKey = (gameId) => {
  return generateCacheKey(CACHE_PREFIXES.SYSTEM_REQUIREMENTS, gameId)
}
