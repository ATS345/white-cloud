...
describe('OrdersService', () => {
  ...
  describe('createOrder', () => {
    ...
    it('should throw BadRequestException if game not found', async () => {
      ...
      await expect(service.createOrder(1, { gameIds: [999] })).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw ConflictException if game already purchased', async () => {
      ...
      await expect(service.createOrder(1, { gameIds: [1] })).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('cancelOrder', () => {
    ...
    it('should throw BadRequestException if order not pending', async () => {
      mockPrismaService.order.findFirst.mockResolvedValue({ ...mockOrder, status: OrderStatus.PAID });
