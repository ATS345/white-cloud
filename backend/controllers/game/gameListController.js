...
// 从models中获取所有需要的模型
const {
  Game, GameCategory, GameTag, GameSystemRequirement, Developer,
} = models;
...
export const getGames = async (req, res) => {
  try {
    ...
    // 计算总页数
    const totalPages = Math.ceil(games.count / parseInt(limit, 10));
...
export const getGameById = async (req, res) => {
  try {
    ...
    // 使用缓存查询
    const game = await cacheQuery(cacheKey, CACHE_EXPIRY.LONG, async () => Game.findByPk(id, {
      include: [
        {
          model: GameCategory,
          as: 'categories',
