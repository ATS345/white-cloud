...
// 从models中获取所有需要的模型
const { Game, GameCategory, GameTag, GameVersion, GameSystemRequirement, Developer } = models
...
export const createGameVersion = async (req, res) => {
  try {
    ...
    // 6. 记录版本创建日志
    logger.info(`Developer ${developer.id} created version ${version_number} for game ${game.id} at ${new Date().toISOString()}`)
