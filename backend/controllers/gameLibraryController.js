...
export const getUserGameLibrary = async (req, res) => {
  try {
    ...
    const { page = 1, limit = 20, sortBy = 'purchase_date', sortOrder = 'desc' } = req.query
...
export const getGameLibraryStats = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      message: '获取游戏库统计信息成功',
      data: {
        totalGames: parseInt(stats.dataValues.total_games) || 0,
        totalPlaytime: parseInt(stats.dataValues.total_playtime) || 0,
...
export const updatePlaytime = async (req, res) => {
  try {
    ...
    await gameLibrary.update({
      playtime: gameLibrary.playtime + parseInt(additionalPlaytime),
