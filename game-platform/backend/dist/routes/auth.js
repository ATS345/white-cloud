...
const validateLoginInput = (req, res) => {
...
const validateChangePasswordInput = (req, res) => {
...
router.post('/register', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // 哈希密码
        const passwordHash = yield bcrypt_1.default.hash(password, BCRYPT_SALT_ROUNDS);
        // 创建新用户
        const newUser = {
            id: nextUserId++,
            username,
        ...
        // 生成JWT token
        const token = jsonwebtoken_1.default.sign({ id: newUser.id, username: newUser.username, email: newUser.email }, JWT_SECRET, jwtOptions);
        // 优化：移除敏感信息
        const { passwordHash: _ } = newUser, userWithoutPassword = __rest(newUser, ["passwordHash"]);
        res.status(201).json(Object.assign(Object.assign({}, userWithoutPassword), { token }));
    }
    catch (error) {
...
router.post('/login', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // 生成JWT token
        const token = jsonwebtoken_1.default.sign({ id: user.id, username: user.username, email: user.email }, JWT_SECRET, jwtOptions);
        // 移除敏感信息
        const { passwordHash: _ } = user, userWithoutPassword = __rest(user, ["passwordHash"]);
        res.status(200).json(Object.assign(Object.assign({}, userWithoutPassword), { token }));
    }
    catch (error) {
...
// Update user route
router.put('/update', auth_1.authMiddleware, (req, res) => {
    var _a;
    ...
    try {
        ...
        // 更新用户
        const updatedUser = Object.assign(Object.assign({}, user), { username: username || user.username, bio: bio || user.bio, avatar: avatar || user.avatar, updatedAt: new Date().toISOString() });
        // 更新Map
        ...
        // 移除敏感信息
        const { passwordHash: _ } = updatedUser, userWithoutPassword = __rest(updatedUser, ["passwordHash"]);
        res.status(200).json(userWithoutPassword);
    }
    catch (error) {
...
router.put('/change-password', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // 哈希新密码
        const newPasswordHash = yield bcrypt_1.default.hash(newPassword, BCRYPT_SALT_ROUNDS);
        // 更新密码
        const updatedUser = Object.assign(Object.assign({}, user), { passwordHash: newPasswordHash, updatedAt: new Date().toISOString() });
        // 更新Map
        usersMap.set(userId, updatedUser);
